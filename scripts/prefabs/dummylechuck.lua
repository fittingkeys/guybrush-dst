...
local assets =
{
    Asset("ANIM", "anim/dummylechuck.zip"),
    Asset("ATLAS", "images/inventoryimages/dummylechuck.xml"),
    Asset("IMAGE", "images/inventoryimages/dummylechuck.tex"),

    Asset("MINIMAP_IMAGE", "dummylechuck"),
}
...
local function do_digits(inst, number)
    ...
    for digit_index = NUM_DIGITS, 1, -1 do
        ...
        inst.AnimState:OverrideSymbol("column"..digit_index, "dummylechuck", "number"..digit.."_black")
    end
...
local function do_hit_presentation(inst)
    if not inst:HasTag("burnt") then
        ...
        if inst.SoundEmitter then
            ...
            -- Sound sequence (1 → 2 → 3 → 1 …)
            if inst.hit_counter == nil then
                inst.hit_counter = 1
            end

            local hit_sounds = {
                [1] = "dummyhit/custom/hit1",
                [2] = "dummyhit/custom/hit2",
                [3] = "dummyhit/custom/hit3"
            }

            local sound_id = "hit_" .. tostring(inst.hit_counter)


            inst.SoundEmitter:KillSound(sound_id)  -- Stop old sound
            inst.SoundEmitter:PlaySound(hit_sounds[inst.hit_counter], sound_id)
            inst.SoundEmitter:SetVolume(sound_id, 0.1)

            -- Increment counter (1 → 2 → 3 → back to 1)
            inst.hit_counter = inst.hit_counter + 1
            if inst.hit_counter > 3 then
                inst.hit_counter = 1
            end
        end
    end
...
local function basefn()
    ...
    inst.AnimState:SetBank("dummylechuck")
...
return Prefab("dummylechuck", defaultfn, assets, prefabs),
    MakePlacer("dummylechuck_placer", "dummylechuck", "dummylechuck", "placer")