...
function MakeBumperType(data)
    ...
    local function fn()
        ...
        -- Do NOT add a standard repair component for the Guybrush bumper!
        if data.name ~= "guybrush" then
        ...
        if data.name == "guybrush" then
            inst:AddTag("boat_bumper_guybrush")
            
            -- Use standard repairable component
            local repairable = inst:AddComponent("repairable")
            repairable.repairmaterial = "gummi"
            repairable.onrepaired = onrepaired
            repairable.testvalidrepairfn = ValidRepairFn
        end
...
local boatbumperdata =
{
    ...
    -- Guybrush Bumper (like wood, but with its own build)
    {
        name = "guybrush",
        material = "gummi",
        tags = {"guybrush"},
        ...
        flammable = false,
        buildsound = "dontstarve/common/place_structure_wood",
        kitpostinit = function(inst)
            if inst and inst.components and inst.components.inventoryitem then
                inst.components.inventoryitem.atlasname = "images/inventoryimages/boat_bumper_guybrush_kit.xml"
                inst.components.inventoryitem.imagename = "boat_bumper_guybrush_kit"
            end
        end,
        buildname = "boat_bumper_guybrush",
        hitfx = "boat_bumper_hit_guybrush"
    },
}
local function MakeBumperHitFX(name, build)
    local assets = {
        Asset("ANIM", "anim/boat_bumper.zip"),
    }
    return Prefab("boat_bumper_hit_"..name, function()
        local inst = CreateEntity()
        ...
        inst.AnimState:SetBank("boat_bumper")
        inst.AnimState:SetBuild(build or "boat_bumper")
        inst.AnimState:PlayAnimation("hit_fx")
...
for _, v in ipairs(boatbumperdata) do
    ...
    table.insert(boatbumperprefabs, MakeBumperHitFX(v.name, v.buildname))
end
